...
export const getAllPosts = async () => {
    return await getDocs(collection(db, 'postsOther')).then((snaps) => snaps.docs.map((d) => d.data()))
}
...
export const getAllPostsWithCategory = async (categoryId) => {
    const q = query(collection(db, 'postsOther'), where('categoryId', '==', categoryId))
    return await getDocs(q).then((snaps) => snaps.docs.map((d) => d.data()))
}

export const getPosts = async (id) => {
    return await getDoc(doc(db, `postsOther/${id}`)).then((snap) => snap.data());
}